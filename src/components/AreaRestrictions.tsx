...
const services = [
  ...
  { title: "Speakers", description: "High-quality speaker systems and audio equipment for any venue." },
  { title: "DJ Services", description: "Professional DJ services with modern equipment and curated playlists." },
  { title: "Decoration Events", description: "Premium event decoration including floral, lighting, and thematic designs." },
];

const AreaRestrictions = () => (
  <section className="py-24 px-6">
    <div className="max-w-6xl mx-auto">
      <h2 className="font-heading text-4xl sm:text-5xl text-center gold-text mb-4">Area Restrictions</h2>
      <p className="text-center text-muted-foreground mb-16 font-body">Select services limited to specific regions</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {services.map((service, i) => (
          ...
          >
            ...
            <p className="text-muted-foreground font-body text-sm leading-relaxed mb-5 flex-1">{service.description}</p>
            <div className="flex items-center gap-2 px-3 py-2 rounded bg-primary/10 border border-primary/30">
              <AlertTriangle size={14} className="gold-text shrink-0" />
              <span className="text-xs font-body gold-text leading-tight">
                Available only in Madurai and surrounding areas
              </span>
            </div>
          </div>
        ))}
