...
import {
  ...
  AlertCircle, Package, Plus, FolderPlus, Folder, Edit2,
} from "lucide-react";
...
const AdminPage = () => {
  ...
  const loadGroups = async () => {
    const { data } = await supabase.from("gallery_groups").select("*").order("sort_order", { ascending: true });
    setGroups(data || []);
  };
  ...
  useEffect(() => { loadMedia(); loadGroups(); loadProducts(); }, []);
  useEffect(() => {
    if (tab === "analytics") { loadStats(); loadFeedback(); }
  }, [tab]);
  ...
  const handleAssignGroup = async () => {
    ...
    setMedia((prev) => prev.map((m) => m.id === assignMedia.id ? { ...m, group_id: assignGroupId || null } : m));
    setAssignMedia(null);
  };
  ...
  const handleCreateGroup = async () => {
    ...
    setSavingGroup(true);
    const slug = groupForm.name.trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
  ...
  // Filtered media for gallery tab
          ...
          <div className="space-y-8 pb-16">
            ...
            <div className="bg-card border gold-border rounded-lg p-6 gold-glow">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-heading text-xl gold-text">Gallery Groups</h2>
                <button
              ...
              {showGroupForm && (
                <div className="mb-5 p-4 bg-secondary/40 rounded-lg border gold-border space-y-3">
                  <div>
                    <label className="text-xs text-muted-foreground uppercase tracking-wider mb-1 block font-body">Group Name (shown to users)</label>
                    <input
                      type="text"
              ...
              {/* Groups list */}
              {groups.length === 0 ? (
                <p className="text-sm text-muted-foreground font-body">No groups yet. Create one to organise your gallery.</p>
              ) : (
            ...
            <div className="bg-card border gold-border rounded-lg p-6 gold-glow">
              ...
              {groups.length > 0 && (
                <div className="mb-4">
                  <label className="text-xs text-muted-foreground uppercase tracking-wider mb-1.5 block font-body">Upload to Group (optional)</label>
                  <select
                    value={uploadGroupId}
                  ...
                  >
                    <option value="">No group (show in All)</option>
                    {groups.map((g) => (
            ...
            <div>
              ...
              {loadingMedia ? (
                ...
                <div className="text-center py-16 text-muted-foreground font-body">No media in this group.</div>
              ) : (
            ...
            {addingProduct && (
              <div className="bg-card border gold-border rounded-lg p-6 gold-glow">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  >
                    {productFilePreview ? (
                      <img src={productFilePreview} alt="preview" className="w-full h-full object-cover" style={{ aspectRatio: "5/7" }} />
                    ) : (
                      <>
            ...
            {loadingStats ? (
              ...
              <>
                ...
                <div className="bg-card border gold-border rounded-lg p-6 gold-glow">
                  ...
                  <p className="text-xs text-muted-foreground font-body mb-5">All feedback submitted by users</p>
                  {loadingFeedback ? (
      ...
      {assignMedia && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4"
          ...
          <div className="bg-card border gold-border rounded-xl p-6 max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-heading text-lg gold-text mb-4">Assign to Group</h3>
