    ...
    <div className="min-h-screen bg-background">
      ...
      <Modal open={aboutOpen} onClose={() => setAboutOpen(false)} title="About Us">
        <div className="space-y-4 text-secondary-foreground font-body text-sm leading-relaxed">
          ...
          <p>Our specialized inventory includes export-quality inflatable costumes available for both sale and rental, ensuring a unique and high-standard production for every client.</p>
          <p>Based in Madurai, we bring over three decades of expertise, cultural understanding, and unwavering commitment to excellence to every celebration we touch.</p>
        </div>
      </Modal>
      ...
      <Modal open={supportOpen} onClose={() => setSupportOpen(false)} title="Contact & Support">
        <div className="space-y-5 font-body text-sm">
          ...
          <p className="text-[10px] text-muted-foreground/50 pt-4 text-center">Website designed and developed by Dalbert Joe</p>
        </div>
      </Modal>
      ...
      <Modal open={feedbackOpen} onClose={() => setFeedbackOpen(false)} title="Feedback">
        <form className="space-y-4 font-body" onSubmit={(e) => e.preventDefault()}>
          <div>
            ...
            <input type="text" className="w-full bg-secondary border gold-border rounded px-4 py-2.5 text-sm text-secondary-foreground focus:outline-none focus:border-primary transition-colors cursor-none" placeholder="Your name" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground uppercase tracking-wider mb-1 block">Phone Number</label>
            <input type="tel" className="w-full bg-secondary border gold-border rounded px-4 py-2.5 text-sm text-secondary-foreground focus:outline-none focus:border-primary transition-colors cursor-none" placeholder="Your phone number" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground uppercase tracking-wider mb-1 block">Message</label>
            <textarea rows={4} className="w-full bg-secondary border gold-border rounded px-4 py-2.5 text-sm text-secondary-foreground focus:outline-none focus:border-primary transition-colors resize-none cursor-none" placeholder="Your message" />
          </div>
          <button type="submit" className="w-full py-3 bg-primary text-primary-foreground font-medium rounded hover:opacity-90 transition-opacity cursor-none">
            Submit Feedback
          </button>
        </form>
      </Modal>
